...
use crate::{
    AddrNetwork, AddrRecord, Instruction, Operation, Program, ProgramBuilder, TaprootKeypair,
...
pub struct Compiler {
    ...
    variables: Vec<Box<dyn Any + Send>>,
    output: CompiledProgram,
    /// Tracks the number of instructions already compiled (for incremental compilation)
    last_compiled_instruction: usize,
}
...
/// Marker type for time variables. When consumed, resolves to current system time.
#[derive(Clone, Debug)]
struct LiveTime;
...
impl Compiler {
    ...
    pub fn new() -> Self {
        Self {
            ...
            last_compiled_instruction: 0,
        }
    }

    /// Compile instructions from the builder starting at the last compiled instruction.
    /// Returns only the NEW actions produced by this compilation batch.
    /// Takes ProgramBuilder directly to avoid cloning via finalize().
    pub fn compile_from(
        &mut self,
        builder: &ProgramBuilder,
    ) -> Result<Vec<CompiledAction>, CompilerError> {
        let from_instruction = self.last_compiled_instruction;
        let actions_before = self.output.actions.len();

        for instruction in builder.instructions.iter().skip(from_instruction) {
            let inst_actions_before = self
                .output
                .actions
                .iter()
                .filter(|action| !matches!(action, CompiledAction::Probe))
            ...
            for _ in inst_actions_before..inst_actions_after {
                self.output
                    .metadata
                    .action_indices
                    .push(self.output.metadata.instructions);
        ...
        // Update tracking of compiled instructions
        self.last_compiled_instruction = builder.instructions.len();
    ...
    fn compile_instruction(&mut self, instruction: &Instruction) -> Result<(), CompilerError> {
        match instruction.operation.clone() {
            Operation::Nop { .. }
            | Operation::LoadNode(..)
            | Operation::LoadConnection(..)
            ...
            Operation::BeginWitnessStack | Operation::AddWitness | Operation::EndWitnessStack => {
                self.handle_witness_operations(instruction)?;
            }
    ...
    fn handle_load_operation<T: 'static + Send>(&mut self, value: T) {
    ...
    fn handle_load_operations(&mut self, instruction: &Instruction) -> Result<(), CompilerError> {
        match &instruction.operation {
            ...
            Operation::LoadTime(_) => {
                // Store a LiveTime marker that resolves to current time when consumed
                self.handle_load_operation(LiveTime);
            }
    ...
    fn handle_time_operations(&mut self, instruction: &Instruction) -> Result<(), CompilerError> {
        match &instruction.operation {
            Operation::AdvanceTime | Operation::SetTime => {
                // No-op: we use system clock, no mock time manipulation
            }
    ...
    /// Helper to get a timestamp from a time variable (resolves LiveTime to current system time)
    fn get_timestamp_input(
        &self,
        inputs: &[usize],
        index: usize,
    ) -> Result<u64, CompilerError> {
        let live_time = self.get_input::<LiveTime>(inputs, index)?;
        Ok(live_time.get_timestamp())
    }
    ...
    fn append_variable<T: 'static + Send>(&mut self, value: T) {
        self.output
            .metadata
            .variable_indices
            .push(self.output.metadata.instructions);
    ...
    fn build_block(&mut self, instruction: &Instruction) -> Result<(), CompilerError> {
        ...
        let time_var = self.get_timestamp_input(&instruction.inputs, 2)?;
